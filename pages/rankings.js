...
export async function getStaticProps() {
  ...
  // Weekly summary is Portable Text – fallback to empty array.
...
export default function Rankings({ rankings, week, summary, scoreExplanation, legislatorsCache }) {
  ...
  const pageTitle = `Progressive Power Rankings – Week ${week ? new Date(week).toLocaleDateString() : "N/A"}`;
  ...
  return (
    <div className={styles.container}>
          ...
          filteredRankings.map((p) => (
            <div key={p._id} className={styles.rankingRow} onClick={() => toggle(p._id)}>
              ...
              <div className={styles.readMoreContainer}>
                <button className={styles.readMoreButton} onClick={(e) => { e.stopPropagation(); toggle(p._id); }}>
              ...
              {expanded === p._id && (
                <div className={styles.expandedRow}>
                  <p className={styles.expandedSummary}>
                    {p.summary || "No individual summary available."}
                  </p>
      ...
      {scoreExplanation && scoreExplanation.content && (
